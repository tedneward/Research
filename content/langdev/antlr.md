title=ANTLR
tags=tool, language, jvm, clr, native, language development
summary=Code-generating parser tool
~~~~~~

[Website](http://www.antlr.org) | [Github](https://github.com/antlr/antlr4) | [4.8 Docs](https://github.com/antlr/antlr4/blob/4.8/doc/index.md) | [ANTLR Lab](http://lab.antlr.org/)

## Grammars

* Collection of GitHub [grammars](https://github.com/antlr/grammars-v4).
* [E](http://erights.org/e-impls/e-on-e/egrammar/index.html) grammar

## Implementations

* [demo-antlr4-llvm](https://github.com/quantumsheep/demo-antlr4-llvm)
* [compiler-example](https://github.com/klebermagno/compiler)

[Gradle plugin](https://github.com/melix/antlr4-gradle-plugin): Antlr4 Gradle Plugin (archived)

## Reading

- [Adaptive LL(*) Parsing: The Power of Dynamic Analysis](http://www.antlr.org/papers/allstar-techreport.pdf)
- [How to Create a Programming Language Using ANTLR4](https://progur.com/2016/09/how-to-create-language-using-antlr4.html) | 
- [A Simple ANTLR Parser](https://courses.ics.hawaii.edu/ReviewICS312/morea/Compiling/reading-ANTLR_grammar.html)
- [Easy intro to writing a BASIC compiler](https://mateiw.github.io/antlr-intro/)
- [The Language from the Dragon Book in ANTLR](https://www1.cs.columbia.edu/~sedwards/classes/2006/w4115-fall/dragonbook-language.pdf)

Strumenta:

* [Parsing Any Language in Java in 5 Minutes Using ANTLR: for Example Python](https://tomassetti.me/parsing-any-language-in-java-in-5-minutes-using-antlr-for-example-python/)
* [The ANTLR Mega Tutorial](https://tomassetti.me/antlr-mega-tutorial/)

## ANTLR 3

[Pie](https://theantlrguy.atlassian.net/wiki/spaces/ANTLR3/pages/2686992/Pie)

[Fig](https://theantlrguy.atlassian.net/wiki/spaces/ANTLR3/pages/2687365/Fig+-+Generic+configuration+language+interpreter)

